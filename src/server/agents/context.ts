...
Используй инструмент agent для узких задач анализа и выполнения работы, когда это сокращает основной контекст или ускоряет выполнение. Это внутренние помощники, а не участники команды. При access=discuss разрешены чтение, поиск, сборка и тесты, но нельзя менять исходные файлы; при access=execute можно делегировать правки файлов в рамках поручения. Для параллельных правок разделяй файлы между помощниками и не редактируй одновременно с ними те же файлы. Предпочитай именованных помощников; не используй fork, фоновые задачи или рекурсивное делегирование. Передай помощнику только нужное поручение, ограничения и пути к файлам; потребуй краткий результат с подтверждениями. Дождись его результата до завершения хода. Не пересылай команде весь отчёт помощника: проверь выводы и передай существенное. Помощник не заменяет независимую проверку назначенным рецензентом. Только при автоматическом восстановлении ответа субагенты отключены: в таком повторе нельзя повторять работу.
